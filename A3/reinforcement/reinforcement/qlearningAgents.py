...
class QLearningAgent(ReinforcementAgent):
    ...
    def getAction(self, state):
        ...
        "*** YOUR CODE HERE ***"
        valueAndAction = util.Counter()
        randomActionProb = util.flipCoin(self.epsilon)
        ...
        if len(legalActions) != 0:
            if randomActionProb:
                ...
                for legalAction in legalActions:
                    valueAndAction[legalAction] = self.getQValue(state, legalAction)

                action = valueAndAction.argMax()
        else:
            action = None
    ...
    def update(self, state, action, nextState, reward):
        ...
        nextQVals = [self.getQValue(nextState, nextAction) for nextAction in nextActions]

        if len(nextQVals) != 0:
            maxNextQVal = max(nextQVals)
        else:
            maxNextQVal = 0.0

        sample = reward + self.discount*maxNextQVal
